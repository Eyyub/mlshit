...
def generator(zs, ys, G_wdim):
    ...
    with tf.name_scope('G'):
        with tf.name_scope('z'):
            G['W_z'] = tf.Variable(tf.truncated_normal([get_shape(zs)[-1], G_wdim['W_z']], stddev=0.1))
            G['z_z'] = tf.matmul(zs, G['W_z'])
            G['a_z'] = tf.nn.dropout(lkrelu(G['z_z']), 1.0)

        with tf.name_scope('y'):
            G['W_y'] = tf.Variable(tf.truncated_normal([get_shape(ys)[-1], G_wdim['W_y']], stddev=0.1))
            G['z_y'] = tf.matmul(ys, G['W_y'])
            G['a_y'] = tf.nn.dropout(lkrelu(G['z_y']), 1.0)
            tf.summary.histogram('W', G['W_y'])
            tf.summary.histogram('z', G['z_y'])
            tf.summary.histogram('a', G['a_y'])

        with tf.name_scope('concat_zy'):
            G['c_zy'] = tf.concat([G['a_z'], G['a_y']], 1)

        with tf.name_scope('L_1'):
            G['W_1'] = tf.Variable(tf.truncated_normal([get_shape(G['c_zy'])[-1], G_wdim['W_1']], stddev=0.1))
            G['z_1'] = tf.matmul(G['c_zy'], G['W_1'])
            G['a_1'] = tf.nn.dropout(lkrelu(G['z_1']), 1.0)

        with tf.name_scope('L_o'):
            G['W_o'] = tf.Variable(tf.truncated_normal([get_shape(G['a_1'])[-1], G_wdim['W_o']], stddev=0.1))
...
def discriminator(real_tensor, fake_tensor, D_wdim):
    ...
    with tf.name_scope('D'):
        with tf.name_scope('x'):
            D['W_x'] = tf.Variable(tf.truncated_normal([get_shape(real_xs)[-1], D_wdim['W_x']], stddev=0.1))
            D['rz_x'] = tf.matmul(real_xs, D['W_x'])
            D['ra_x'] = tf.nn.dropout(lkrelu(D['rz_x']), 1.0)
            D['fz_x'] = tf.matmul(fake_xs, D['W_x'])
            D['fa_x'] = tf.nn.dropout(lkrelu(D['fz_x']), 1.0)

        with tf.name_scope('y'):
            D['W_y'] = tf.Variable(tf.truncated_normal([get_shape(real_ys)[-1], D_wdim['W_y']], stddev=0.1))
            D['rz_y'] = tf.matmul(real_ys, D['W_y'])
            D['ra_y'] = tf.nn.dropout(lkrelu(D['rz_y']), 1.0)
            D['fz_y'] = tf.matmul(fake_ys, D['W_y'])
            D['fa_y'] = tf.nn.dropout(lkrelu(D['fz_y']), 1.0)
            tf.summary.histogram('W', D['W_y'])
            tf.summary.histogram('fz', D['fz_y'])
            tf.summary.histogram('fa', D['fa_y'])
            tf.summary.histogram('rz', D['rz_y'])
            tf.summary.histogram('ra', D['ra_y'])
        with tf.name_scope('concat_xy'):
            D['rc_xy'] = tf.concat([D['ra_x'], D['ra_y']], 1)
            D['fc_xy'] = tf.concat([D['fa_x'], D['fa_y']], 1)

        with tf.name_scope('L_1'):
            D['W_1'] = tf.Variable(tf.truncated_normal([get_shape(D['rc_xy'])[-1], D_wdim['W_1']], stddev=0.1))
        ...
        with tf.name_scope('L_o'):
            D['W_o'] = tf.Variable(tf.truncated_normal([get_shape(D['ra_1'])[-1], D_wdim['W_o']], stddev=0.1))
...
with tf.device('/gpu:0'):
    #tenter de mettre le meme y que le train btach au lieu d'un y random pour voir
    ...
    G_train_step = tf.train.AdamOptimizer(lr, beta1=0.5).minimize(G_loss, var_list=get_vars(G))
    D_train_step = tf.train.GradientDescentOptimizer(lr).minimize(D_loss, var_list=get_vars(D))
    ...
    for epoch in range(step_epochs):
        for step in range(k):
            ...
            _, dloss, summary = sess.run([D_train_step, D_loss, total_summary],
                                        feed_dict={real_xs: 2. * mini_batch_xs - 1. , real_ys: mini_batch_ys, zs:np.random.randn(batch_size, zdim), g_ys: mini_batch_ys })
            writer.add_summary(summary, summary_nb)
            summary_nb += 1
            print('D loss, epoch %d step %d value %f' % (epoch, step, dloss))

        _, gloss, tb = sess.run([G_train_step, G_loss, G['a_o']], feed_dict={zs:np.random.randn(batch_size, zdim), g_ys: random_one_hot((batch_size, 10))})
        imgs_summary = sess.run(genimg_summary, feed_dict={zs:np.random.randn(10, zdim), g_ys: np.eye(10)})
