...
dataset_X, dataset_Y, labels = np.load('animal_dataset.npy')
dataset_X = 2. * dataset_X - 1.
batch_size = 64
zdim = 100
ydim = len(labels)
...
with tf.device('/gpu:0'):
    ...
    G_deconv_params = {
        ...
        'strides2' : [1, 2, 2, 1],
        'deconv2_shp' : [16, 16, 64],
        'filters3' : tf.Variable(tf.truncated_normal([5, 5, 3, 64], stddev=0.02)),
        'strides3' : [1, 2, 2, 1],
        'deconv3_shp' : [32, 32, 3]
    }

    D_conv_params = { #changer le nom
        'filters1' : tf.Variable(tf.truncated_normal([5, 5, 3, 32], stddev=0.02)),
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
    ...
    for step_epoch in range(50000*4):
        ...
        _, gloss_curr = sess.run([G_train_step, G_loss], feed_dict={zs : t, g_ys: random_one_hot((batch_size, len(labels))),
                                                                    is_g_training : True, is_d_training : True})

        if step_epoch % 500 == 1:
            imgs = sess.run(G['a_o'], feed_dict={zs : np.repeat(np.random.uniform(-1, 1, (len(labels), zdim)), len(labels), axis=0), g_ys: np.tile(np.eye(len(labels)), [len(labels), 1]), is_g_training : True, is_d_training : True})
            fig = plt.figure()
            fig.set_size_inches(10, 10)
            fig.subplots_adjust(left=0, bottom=0,
                                   right=1, top=1, wspace=0, hspace=0.1)
            for i in range(len(labels) ** 2):
                fig.add_subplot(len(labels), len(labels), i+1)
                plt.imshow((imgs[i] + 1.) / 2.)
                plt.axis('off')
            plt.savefig('images_animal/epoch_%d.jpg' % step_epoch, dpi=100)
